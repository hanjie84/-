...
namespace bysj.Tests
{
    public abstract class bysjTestBase : AbpIntegratedTestBase<bysjTestModule>
    {
        ...
        protected void UsingDbContext(Action<bysjDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<bysjDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<bysjDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<bysjDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<bysjDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<bysjDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<bysjDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<bysjDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<bysjDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<bysjDbContext, Task<T>> func)
        {
            T result;
